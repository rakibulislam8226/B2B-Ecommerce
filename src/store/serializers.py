...
class ProductSerializer(serializers.ModelSerializer):
    organization = serializers.SlugRelatedField(
        slug_field="uid", queryset=Organization.objects.filter()
    )
    category = serializers.SlugRelatedField(
        slug_field="uid", queryset=Category.objects.filter()
    )
...
class CartItemSerializer(serializers.ModelSerializer):
    cart = serializers.SlugRelatedField(
        slug_field="uid", queryset=Cart.objects.filter()
    )
    product = serializers.SlugRelatedField(
        slug_field="uid", queryset=Products.objects.filter()
    )
...
class CartSerializer(serializers.ModelSerializer):
    ...
    user = serializers.SlugRelatedField(
        slug_field="uid", queryset=User.objects.filter()
    )
...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    order = serializers.SlugRelatedField(
        slug_field="uid", queryset=Order.objects.filter()
    )
